...
fn stale_policy_is_typed_with_bundle_identity_and_freshness_evidence() {
    ...
    let policy_not_current = PolicyNotCurrent::new(bundle, freshness_evidence);
    let result =
...
fn out_of_jurisdiction_is_typed_with_scope_evidence_not_legal_action_support() {
    ...
    let out_of_jurisdiction = OutOfJurisdiction::new(jurisdiction_evidence, policy_bundle);
    let result =
