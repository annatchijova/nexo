...
- **`policy_bundles`, `policy_bundle_activations`** — bundle identity is
  ...
  each bundle also has at most one activation event, activation requires at
  least one normative claim, and activation timestamps are assigned by the
  database clock.
...
- **`action_evaluations`** — top-level columns plus the sealed JSON payload,
  ...
  have an activation record before an evaluation can be persisted.
