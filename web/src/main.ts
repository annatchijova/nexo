...
function renderLanding(): void {
  ...
  app.innerHTML = `<main class="landing shell">
    <header class="topbar"><div class="brand"><img src="${logoUrl}" alt="NEXO" /><div><span class="eyebrow">${text("A CLEARER PLACE TO BEGIN", "UN LUGAR MÁS CLARO PARA EMPEZAR")}</span><h1>NEXO</h1></div></div><nav class="topbar-actions" aria-label="${text("Main navigation", "Navegación principal")}"><button id="open-guide" class="utility-button" type="button">${text("Step-by-step guide", "Guía paso a paso")}</button><button id="language-toggle" class="utility-button" type="button">${state.language === "en" ? "ES" : "EN"}</button><button id="theme-toggle" class="utility-button" type="button">${state.theme === "dark" ? text("Light mode", "Modo claro") : text("Dark mode", "Modo oscuro")}</button></nav></header>
