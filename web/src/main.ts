...
function render(): void {
  ...
  app.innerHTML = `<main class="shell">
    <header class="topbar"><div class="brand"><img src="${logoUrl}" alt="NEXO" /><div><span class="eyebrow">${text("VERIFIABLE DIGITAL-RIGHTS CASE GRAPH", "GRAFO VERIFICABLE DE DERECHOS DIGITALES")}</span><h1>NEXO</h1></div></div><div class="topbar-actions"><a href="/demo" class="utility-button demo-nav-link">${text("OPEN DEMO", "ABRIR DEMO")}</a><span class="status-dot">${text("Local workspace", "Espacio local")}</span><button id="language-toggle" class="utility-button" type="button">${state.language === "en" ? "ES" : "EN"}</button><button id="theme-toggle" class="utility-button" type="button">${state.theme === "dark" ? text("Light mode", "Modo claro") : text("Dark mode", "Modo oscuro")}</button></div></header>
    <section class="workspace-demo-cta panel"><div><span class="eyebrow">${text("NO TOKEN NEEDED", "NO NECESITÁS TOKEN")}</span><h2>${text("Want to see NEXO work first?", "¿Querés ver cómo funciona NEXO primero?")}</h2><p>${text("Open the large guided DEMO. It is right here, uses no real backend and lets you complete the example cases and download the evidence materials.", "Abrí la DEMO guiada grande. Está acá mismo, no usa el backend real y te permite completar los casos de ejemplo y descargar los materiales de evidencia.")}</p></div><a class="demo-cta-button" href="/demo">${text("OPEN THE DEMO", "ABRIR LA DEMO")}</a></section>
